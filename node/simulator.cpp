// Function 
// input: map, desired speed
// output: updated state, odometry, scan 
// simulator mainly update the states according to the desired speed and st_kinematics of the car
// also it check collision with the obstacles and ouput odometry and scan (by calculate) 
...
public:
    ...
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        state = {.x=0, .y=0, .theta=0, .vx=0, .vy=0, .r=0, .s=0, .vs=0};
        accel = 0.0;
        steer_angle_vel = 0.0;
        desired_speed = 0.0;
        ...
        // Get the topic names
        std::string control_topic, map_topic, scan_topic, pose_topic, gt_pose_topic, 
        pose_rviz_topic, odom_topic, imu_topic;
        n.getParam("control_topic", control_topic);
        ...
        control_sub = n.subscribe(control_topic, 1, &RacecarSimulator::control_callback, this);
    ...
    void obs_callback(const geometry_msgs::PointStamped &msg) {
        ...
        int ind = rc_2_ind(rc[0], rc[1]);  // ind one number to mark obstacle
