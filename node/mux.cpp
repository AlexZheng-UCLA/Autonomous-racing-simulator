// Function 
// input: mux controller vector, joystick/keyboard  
// output: drive_st_msg(state and time stamp)
// mux publish desired speed and steering according to the mux controller, but only joystick & keyboard  
...
private:
    ...
    friend class Channel; // Channel can access Mux member
...
public:
    Mux() {
        ...
        add_channel(rand_drive_topic, drive_topic, random_control_mux_idx);
        ...
        std::string autodrive_topic;
        n.getParam("new_drive_topic", autodrive_topic);
        n.getParam("new_mux_idx", autodrive_mux_idx);
        add_channel(autodrive_topic, drive_topic, autodrive_mux_idx);
    }
    ...
    void joy_callback(const sensor_msgs::Joy & msg) {
        ...
        if (mux_controller[joy_mux_idx]) {
            ...
            double desired_velocity = prev_desired_speed * 0.8;
            if (msg.axes[joy_speed_axis] * desired_velocity >= 0 ){
                desired_velocity = desired_velocity + max_incre * msg.axes[joy_speed_axis];
            }
            else {
                desired_velocity = desired_velocity + 1.5 * max_incre * msg.axes[joy_speed_axis];
            }

            double desired_steer = max_steering_angle * msg.axes[joy_angle_axis];
            
            prev_desired_speed = desired_velocity;
